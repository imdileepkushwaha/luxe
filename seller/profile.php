...
?>
        ...
        <div class="admin-page-head">
          ...
          <div class="admin-page-head__actions">
            <a class="admin-btn admin-btn--primary" href="kyc-details.php">Update KYC & Bank</a>
          </div>
        </div>
        ...
        <div class="seller-kpi">
          <div class="seller-kpi-card seller-kpi-card--products">
            <div>
              ...
              <div class="seller-kpi-card__hint">Last update: <?= h((string) ($profile['kyc_updated_at'] ?: '-')) ?></div>
            </div>
          ...
          <div class="seller-kpi-card seller-kpi-card--revenue">
            <div>
              ...
              <div class="seller-kpi-card__hint">Reviewed: <?= h((string) ($profile['kyc_final_reviewed_at'] ?: '-')) ?></div>
            </div>
            <div class="seller-kpi-card__icon" aria-hidden="true">
          ...
          <div class="seller-kpi-card seller-kpi-card--orders">
            <div>
              ...
              <div class="seller-kpi-card__value"><?= h((string) ($profile['kyc_edit_request_status'] ?? 'none')) ?></div>
              <div class="seller-kpi-card__hint">Requested: <?= h((string) ($profile['kyc_edit_requested_at'] ?: '-')) ?></div>
            </div>
            <div class="seller-kpi-card__icon" aria-hidden="true">
        ...
        <?php if (trim((string) ($profile['kyc_rejection_reason'] ?? '')) !== ''): ?>
          <div class="seller-alert seller-alert--warn" style="margin-top:14px">
            Last KYC review note: <?= h((string) $profile['kyc_rejection_reason']) ?>
          </div>
        <?php endif; ?>
        <?php if (trim((string) ($profile['kyc_edit_rejection_reason'] ?? '')) !== ''): ?>
          <div class="seller-alert seller-alert--warn" style="margin-top:10px">
            Last edit request note: <?= h((string) $profile['kyc_edit_rejection_reason']) ?>
          </div>
        <?php endif; ?>

        <div class="card" style="margin-top:16px">
          <div class="card-header">
            <h2 class="card-title">Basic seller details</h2>
          </div>
          <div class="card-body">
            <div class="seller-detail-grid">
              <div class="seller-detail-item"><span>Full name</span><strong><?= h((string) ($profile['full_name'] ?? '-')) ?></strong></div>
              <div class="seller-detail-item"><span>Email</span><strong><?= h((string) ($profile['email'] ?? '-')) ?></strong></div>
              <div class="seller-detail-item"><span>Business name</span><strong><?= h((string) ($profile['business_name'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>Phone number</span><strong><?= h((string) ($profile['phone_number'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item seller-detail-item--wide"><span>Business address</span><strong><?= h((string) ($profile['business_address'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>Allowed categories</span><strong><?= h($allowedCategories !== [] ? implode(', ', $allowedCategories) : '-') ?></strong></div>
              <div class="seller-detail-item"><span>GST number</span><strong><?= h((string) ($profile['gst_number'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>PAN number</span><strong><?= h((string) ($profile['pan_number'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>Aadhaar number</span><strong><?= h((string) ($profile['aadhaar_number'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>ID proof</span><strong><?= h((string) ($profile['id_proof_type'] ?: '-')) ?> / <?= h((string) ($profile['id_proof_number'] ?: '-')) ?></strong></div>
            </div>
          </div>
        </div>
        ...
        <div class="card" style="margin-top:16px">
          <div class="card-header">
            <h2 class="card-title">Branding details</h2>
          </div>
          <div class="card-body">
            <div class="seller-detail-grid">
              <div class="seller-detail-item">
                ...
                <strong>
                  <?php if ((string) ($profile['logo_path'] ?? '') !== ''): ?>
                    <img src="../<?= h((string) $profile['logo_path']) ?>" alt="Seller logo" class="seller-brand-media seller-brand-media--logo">
                  <?php else: ?>
                    -
                  <?php endif; ?>
                </strong>
              </div>
              <div class="seller-detail-item seller-detail-item--wide">
                <span>Current banner</span>
                <strong>
                  <?php if ((string) ($profile['banner_path'] ?? '') !== ''): ?>
                    <img src="../<?= h((string) $profile['banner_path']) ?>" alt="Seller banner" class="seller-brand-media seller-brand-media--banner">
                  <?php else: ?>
                    -
                  <?php endif; ?>
                </strong>
        ...
        <div class="card" style="margin-top:16px">
          ...
          <div class="card-body">
            <form method="post" enctype="multipart/form-data" class="seller-withdraw-form">
              <input type="hidden" name="action" value="save_profile_details">
              <div class="seller-form__row">
                <div>
                  <label for="phone_number">Phone number</label>
                  <input id="phone_number" class="seller-badge-input" type="text" name="phone_number" maxlength="40" placeholder="e.g. +91 9876543210" value="<?= h((string) ($profile['phone_number'] ?? '')) ?>">
                </div>
                <div>
                  <label for="business_address">Business address</label>
                  <input id="business_address" class="seller-badge-input" type="text" name="business_address" maxlength="255" placeholder="Shop / Office address" value="<?= h((string) ($profile['business_address'] ?? '')) ?>">
                </div>
              </div>
              <div class="seller-form__row">
                <div>
                  <label for="logo_file">Upload logo</label>
                  <input id="logo_file" class="seller-badge-input" type="file" name="logo_file" accept=".jpg,.jpeg,.png,.webp,.gif,image/*">
                  <p class="seller-help" style="margin-top:6px">Suggested: square image, max 5MB.</p>
                </div>
                <div>
                  <label for="banner_file">Upload banner</label>
                  <input id="banner_file" class="seller-badge-input" type="file" name="banner_file" accept=".jpg,.jpeg,.png,.webp,.gif,image/*">
                  <p class="seller-help" style="margin-top:6px">Suggested: wide image, max 5MB.</p>
                </div>
              </div>
              <div class="seller-actions">
                <button class="admin-btn admin-btn--primary" type="submit">Save profile details</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div class="card" style="margin-top:16px">
          ...
          <div class="card-body">
            <div class="seller-detail-grid">
              <div class="seller-detail-item"><span>Bank name</span><strong><?= h((string) ($profile['bank_name'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>Account holder</span><strong><?= h((string) ($profile['bank_account_name'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>Account number</span><strong><?= h((string) ($profile['bank_account_number'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>IFSC</span><strong><?= h((string) ($profile['bank_ifsc'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item seller-detail-item--wide"><span>Address</span><strong><?= h((string) ($profile['address_line1'] ?: '-')) ?>, <?= h((string) ($profile['city'] ?: '-')) ?>, <?= h((string) ($profile['state'] ?: '-')) ?> - <?= h((string) ($profile['pin_code'] ?: '-')) ?></strong></div>
              <div class="seller-detail-item"><span>GST document</span><strong>
                <?php if ((string) ($profile['gst_doc_path'] ?? '') !== ''): ?>
                  <a class="seller-preview-btn" href="../<?= h((string) $profile['gst_doc_path']) ?>" target="_blank" rel="noopener">Open</a>
                <?php else: ?>
                  -
                <?php endif; ?>
              </strong></div>
              <div class="seller-detail-item"><span>PAN document</span><strong>
                <?php if ((string) ($profile['pan_doc_path'] ?? '') !== ''): ?>
                  <a class="seller-preview-btn" href="../<?= h((string) $profile['pan_doc_path']) ?>" target="_blank" rel="noopener">Open</a>
                <?php else: ?>
                  -
                <?php endif; ?>
              </strong></div>
              <div class="seller-detail-item"><span>Aadhaar document</span><strong>
                <?php if ((string) ($profile['aadhaar_doc_path'] ?? '') !== ''): ?>
                  <a class="seller-preview-btn" href="../<?= h((string) $profile['aadhaar_doc_path']) ?>" target="_blank" rel="noopener">Open</a>
                <?php else: ?>
                  -
                <?php endif; ?>
              </strong></div>
