    ...
    <section class="profile-shell" aria-label="Profile content">
      <aside class="profile-side">
        ...
        <ul class="profile-menu">
          <li><a href="#" class="is-active" data-tab-link="dashboard">
            <span class="profile-menu-icon">🏠</span> Dashboard
          </a></li>
          <li><a href="#" data-tab-link="addresses">
            <span class="profile-menu-icon">📍</span> Addresses
          </a></li>
          <li><a href="#" data-tab-link="wishlist">
            <span class="profile-menu-icon">💖</span> Wishlist
          </a></li>
          <li><a href="#" data-tab-link="reviews">
            <span class="profile-menu-icon">⭐</span> Reviews
          </a></li>
          <li><a href="#" data-tab-link="rewards">
      ...
      <article class="profile-main">
        ...
        <div class="t1-tab-panel hidden" id="tab-rewards">
          ...
          <div class="t1-rewards-hero" style="background: linear-gradient(135deg, #0f172a, #1e293b); padding: 40px; border-radius: 20px; color: #fff; margin-bottom: 32px; display: flex; align-items: center; gap: 40px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
            <div style="flex-shrink: 0; width: 140px; height: 140px; border-radius: 50%; background: linear-gradient(135deg, #3b82f6, #ec4899); padding: 4px;">
              <div style="width: 100%; height: 100%; border-radius: 50%; background: #0f172a; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                <strong id="rewardsPtsCircle" style="font-size: 32px; font-weight: 800; line-height: 1;"><?= h(number_format($loyaltyBalance)) ?></strong>
                <span style="font-size: 14px; color: #94a3b8; font-weight: 600; text-transform: uppercase; margin-top: 4px;">Points</span>
              </div>
            </div>
            <div style="flex: 1;">
              <h3 id="rewardsTierTitle" style="font-size: 28px; font-weight: 700; margin: 0 0 8px;"><?= h($rewardsTierTitle) ?></h3>
              <p id="rewardsLeadLine" style="color: #cbd5e1; font-size: 16px; margin: 0 0 24px;"><?= $rewardsLeadHtml ?></p>
              <div style="position: relative; width: 100%;">
                <div style="height: 10px; background: rgba(255,255,255,0.1); border-radius: 99px; overflow: hidden; margin-bottom: 8px;">
                  <div id="rewardsProgressFill" style="height: 100%; width: <?= h((string) round($rewardsProgressPct, 1)) ?>%; background: linear-gradient(90deg, #3b82f6, #ec4899); border-radius: 99px; transition: width 1s ease;"></div>
                </div>
                <div style="display: flex; justify-content: space-between; font-size: 12px; font-weight: 600; color: #94a3b8; text-transform: uppercase;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px;">
            <div style="background: #fff; padding: 24px; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
              <h3 style="font-size: 18px; font-weight: 700; color: #0f172a; margin: 0 0 16px;">Points History</h3>
              <div id="rewardsHistory" style="display: flex; flex-direction: column; gap: 16px; max-height: 250px; overflow-y: auto;">
                <?php if (empty($loyaltyHistoryUi)): ?>
                  <p style="color: #64748b; font-size: 14px;">No points history available yet.</p>
                <?php else: ?>
                  <?php foreach ($loyaltyHistoryUi as $h): ?>
                    <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 12px; border-bottom: 1px solid #f1f5f9;">
                      <div>
                        <div style="font-weight: 600; color: #0f172a; font-size: 14px;"><?= h($h['desc']) ?></div>
                        <div style="color: #64748b; font-size: 12px; margin-top: 4px;"><?= h($h['date']) ?></div>
                      </div>
                      <div style="font-weight: 700; color: #10b981; font-size: 16px;"><?= h($h['pts']) ?></div>
                    </div>
                  <?php endforeach; ?>
                <?php endif; ?>
              </div>
            ...
            <div style="background: #fff; padding: 24px; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
              <h3 style="font-size: 18px; font-weight: 700; color: #0f172a; margin: 0 0 16px;">Redeem Points</h3>
              <p style="color: #475569; font-size: 14px; line-height: 1.5; margin-bottom: 24px;">Convert your LUXE Points into wallet balance. <strong>100 points = ₹110 off</strong> on your next order.</p>
              <div style="display: flex; gap: 12px;">
                <input type="number" id="redeemInput" placeholder="Enter points" min="100" max="<?= max(100, $loyaltyBalance) ?>" step="100" style="flex: 1; padding: 12px 16px; border: 1px solid #cbd5e1; border-radius: 12px; font-size: 15px; outline: none; transition: border-color 0.2s;" />
                <button class="profile-edit-btn" onclick="redeemPoints()" style="padding: 0 24px;">Redeem</button>
              </div>
            </div>
          </div>
        </div>
    ...
    (function () {
      ...
      function renderWishlist() {
        ...
        if (!Array.isArray(items) || items.length === 0) {
          wishlistGrid.innerHTML = '<div class="t1-wishlist-empty"><span class="t1-wishlist-empty__icon">💖</span><h3>Your wishlist is empty</h3><p>Product page se heart icon pe click karke apne पसंदीदा products yahan save karo.</p><span class="t1-wishlist-empty__hint">Start exploring and build your premium collection.</span></div>';
          return;
        }
        ...
        wishlistGrid.innerHTML = items.map(function (w) {
          ...
          return '<div class="t1-wishlist-card"><button type="button" class="t1-wishlist-remove" data-w-remove="' + id + '" aria-label="Remove from wishlist">♡</button><a class="t1-wishlist-link" href="../product.php?id=' + id + '"><span class="t1-wishlist-media">' + media + '</span><div class="t1-wishlist-card-body"><strong>' + name + '</strong><span>LUXE — Premium Collection</span><span class="t1-wishlist-meta">Saved item</span><div class="t1-wishlist-price-row">' + priceHtml + '</div></div></a></div>';
        }).join("");
