...
$cmsReturnT2 = cms_page_get($pdo, 'return_policy', [
...
$siteBrandT2Return = site_brand_name($pdo);
if (!empty($cmsReturnT2['updated_at'])) {
    try {
        $lastUpdated = (new DateTimeImmutable((string) $cmsReturnT2['updated_at']))->format('F j, Y');
    } catch (Throwable $e) {
        /* keep default */
    }
}
...
<head>
  ...
  <title><?= h($cmsReturnT2['hero_title'] !== '' ? $cmsReturnT2['hero_title'] : 'Return Policy') ?> — <?= h($siteBrandT2Return) ?></title>
  <meta name="description" content="<?= h($cmsReturnT2['meta_description']) ?>">
    ...
    <section class="t1-static-hero t1-static-hero--compact" aria-labelledby="t1-return-title">
      <div class="container t1-static-hero-inner t1-static-hero-inner--compact">
        <div class="t1-static-hero-copy">
          <?php if (($cmsReturnT2['hero_kicker'] ?? '') !== ''): ?>
            <p class="t1-static-kicker script-accent"><?= h($cmsReturnT2['hero_kicker']) ?></p>
          <?php endif; ?>
          <h1 id="t1-return-title" class="t1-static-title"><?= h($cmsReturnT2['hero_title'] !== '' ? $cmsReturnT2['hero_title'] : 'Return Policy') ?></h1>
          <p class="t1-static-lead">
            <?= nl2br(h($cmsReturnT2['hero_lead'])) ?>
          </p>
        </div>
      </div>
    </section>
    ...
    <div class="container t1-static-wrap">
      ...
      <div class="t1-legal-layout">
        ...
        <article class="t1-legal-doc">
          ...
          <?php if (trim((string) $cmsReturnT2['body_html']) !== ''): ?>
            <div class="t1-legal-cms-body"><?= $cmsReturnT2['body_html'] ?></div>
          <?php else: ?>
