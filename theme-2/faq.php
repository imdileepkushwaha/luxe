...
$cmsFaqPageT2 = cms_page_get($pdo, 'faq', [
...
$siteBrandT2Faq = site_brand_name($pdo);
$cmsFaqRowsT2 = cms_faqs_all($pdo, true);
$faqItems = [];
foreach ($cmsFaqRowsT2 as $row) {
...
<head>
  ...
  <title><?= h($cmsFaqPageT2['hero_title'] !== '' ? $cmsFaqPageT2['hero_title'] : 'FAQ') ?> — <?= h($siteBrandT2Faq) ?></title>
  <meta name="description" content="<?= h($cmsFaqPageT2['meta_description']) ?>">
    ...
    <section class="t1-static-hero t1-static-hero--compact" aria-labelledby="t1-faq-title">
      <div class="container t1-static-hero-inner t1-static-hero-inner--compact">
        <div class="t1-static-hero-copy">
          <?php if (($cmsFaqPageT2['hero_kicker'] ?? '') !== ''): ?>
            <p class="t1-static-kicker script-accent"><?= h($cmsFaqPageT2['hero_kicker']) ?></p>
          <?php endif; ?>
          <h1 id="t1-faq-title" class="t1-static-title"><?= h($cmsFaqPageT2['hero_title'] !== '' ? $cmsFaqPageT2['hero_title'] : 'Frequently asked questions') ?></h1>
          <p class="t1-static-lead">
            <?= nl2br(h($cmsFaqPageT2['hero_lead'])) ?> <a href="contact-us.php" class="t1-inline-link">Contact our team</a>.
          </p>
        </div>
      </div>
    </section>
