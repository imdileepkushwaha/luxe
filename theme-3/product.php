...
<body class="t2-product-page t3-page-pdp">
  ...
  <main>
    ...
    <section class="t1-product-container container">
      ...
      <div class="t1-product-info">
        <div class="t1-product-glass-card pdp-redesigned-card">
          <!-- Sale Tag -->
          <?php
          ...
          <!-- Title -->
          <h1 class="pdp-title"><?= h((string)$product['name']) ?></h1>
          
          <!-- Seller Info -->
          <div class="pdp-seller-row" style="display: flex; align-items: center; gap: 8px; margin-top: 8px; margin-bottom: 6px; font-size: 14px; color: #64748b; font-weight: 500;">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="color: var(--theme-red);"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
            <span>Sold by:</span>
            <a href="seller-store.php?id=<?= (int)$product['seller_id'] ?>" style="color: var(--text); font-weight: 600; text-decoration: none; border-bottom: 1px dashed var(--theme-red); transition: color 0.2s;" onmouseover="this.style.color='var(--theme-red)'" onmouseout="this.style.color='var(--text)'">
              <?= h((string)($product['seller_name'] ?? 'LUXE Store')) ?>
            </a>
          </div>
          ...
          <div class="pdp-rating-row">
            <div class="pdp-stars">
              <?php for($i=1; $i<=5; $i++): ?>
                <?php if($i <= round($displayRating)): ?>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#e11d48"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
                <?php else: ?>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#d1d5db"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
                <?php endif; ?>
              <?php endfor; ?>
            </div>
            <span class="pdp-reviews-count">(<?= h(number_format($displayReviewCount)) ?> Reviews)</span>
          </div>

          <!-- Description Snippet -->
          <div class="pdp-desc-snippet">
            <?php 
              $descRaw = strip_tags((string)($product['description'] ?? ''));
              $descSnippet = mb_strlen($descRaw) > 200 ? mb_substr($descRaw, 0, 200) . '...' : $descRaw;
              if ($descSnippet === '') $descSnippet = 'A premium quality product from our exclusive catalog.';
              echo h($descSnippet);
            ?>
          </div>
          ...
          <div class="pdp-price-stack">
            <?php if (theme1_old_price((array)$product) !== ''): ?>
              <del class="pdp-price-old"><?= h(theme1_old_price((array)$product)) ?></del>
            <?php endif; ?>
            <span class="pdp-price-current"><?= h(theme1_price((array)$product)) ?></span>
          </div>

          <!-- Color Swatches -->
          <?php if (!empty($colorOptions)): ?>
          <div class="pdp-swatch-group">
            <h3 class="pdp-swatch-title">Color</h3>
            <div class="pdp-swatches-list">
              <?php foreach ($colorOptions as $idx => $col): 
                ...
                if ($isLinked):
              ?>
                <a href="<?= h($colorLinkedProducts[$lk]['url']) ?>" class="pdp-color-swatch" 
                   style="background: <?= h(product_swatch_style_for_color($col, $idx)) ?>;"
                   title="<?= h($col) ?>"></a>
              <?php else: ?>
                <button type="button" class="pdp-color-swatch <?= $idx === $activeColorIdx ? 'active' : '' ?>" 
                        style="background: <?= h(product_swatch_style_for_color($col, $idx)) ?>"
                        data-color="<?= h($col) ?>" onclick="selectProductColor(this)" title="<?= h($col) ?>"></button>
              <?php endif; endforeach; ?>
            </div>
            <div style="display:none" id="selectedColorName"><?= h($selectedColorDefault) ?></div>
          </div>
          <?php endif; ?>
          ...
          <div class="pdp-swatch-group">
            <h3 class="pdp-swatch-title">Size</h3>
            <div class="pdp-swatches-list size-list">
              <?php foreach ($sizeOptions as $idx => $sz): ?>
                <button type="button" class="pdp-size-btn <?= $idx === $activeSizeIdx ? 'active' : '' ?>" 
                        data-size="<?= h($sz) ?>" onclick="selectProductSize(this)"><?= h($sz) ?></button>
              <?php endforeach; ?>
            </div>
            <div style="display:none" id="selectedSizeName"><?= h($sizeOptions[$activeSizeIdx] ?? '') ?></div>
            <div id="stockAvailability" class="pdp-stock-msg"></div>
          </div>
          <?php endif; ?>
          ...
          <div class="pdp-actions-row">
            ...
            <button type="button" id="addToCartBtn" class="pdp-btn-add-cart" onclick="addToCartEvent()" <?= $initialSpecStockQty <= 0 ? 'disabled' : '' ?>>
              <?= $initialSpecStockQty > 0 ? 'Add To cart' : 'Out of Stock' ?>
            </button>
            <button
              type="button"
              class="pdp-btn-icon pcard__wish-toggle"
            ...
            <button type="button" class="pdp-btn-icon" aria-label="Share" onclick="openShareModal()">
          ...
          <div class="pdp-meta-list">
            <div class="pdp-meta-item">
              <strong>SKU :</strong> <span><?= h((string)($product['sku'] ?? 'N/A')) ?></span>
            </div>
            <div class="pdp-meta-item">
              <strong>Category :</strong> <span><?= h(ucwords((string)($product['category'] ?? 'General'))) ?></span>
            </div>
            <div class="pdp-meta-item">
              <strong>Tags :</strong> <span><?= h(implode(' , ', array_filter([$product['brand'] ?? '', $product['product_type'] ?? '']))) ?></span>
            </div>
    ...
    <section class="t1-product-tabs-section container">
      <div class="pdp-tabs-layout">
        <!-- Sidebar -->
        <aside class="pdp-tabs-sidebar">
          <div class="pdp-additional-info">
            <h3>Additional Information:</h3>
            <ul>
              <li><span>Brand</span><span><?= h((string)($product['brand'] ?? '-')) ?></span></li>
              <li><span>Model</span><span><?= h((string)($product['name'] ?? '-')) ?></span></li>
              <li><span>Category</span><span><?= h(ucwords((string)($product['category'] ?? '-'))) ?></span></li>
              <li><span>Product type</span><span><?= h(ucwords((string)($product['product_type'] ?? 'General'))) ?></span></li>
              <li><span>Shipping</span><span><?= h(ucwords(str_replace('_', ' ', (string)($product['shipping_class'] ?? 'Standard')))) ?></span></li>
              <li><span>Seller</span><span><a href="seller-store.php?id=<?= (int)$product['seller_id'] ?>"><?= h((string)($product['seller_name'] ?? 'LUXE Store')) ?></a></span></li>
              <li><span>Sizes</span><span><?= !empty($sizeOptions) ? h(implode(', ', $sizeOptions)) : '-' ?></span></li>
              <li><span>Colors</span><span><?= !empty($colorOptions) ? h(implode(', ', $colorOptions)) : '-' ?></span></li>
              <li><span>SKU Code</span><span><?= h((string)($product['sku'] ?? '-')) ?></span></li>
            </ul>
          </div>
        </aside>

        <!-- Main Tabs Area -->
        <div class="pdp-tabs-main">
          ...
          <div class="pdp-tabs-content-wrapper">
            <div id="tab-desc" class="t1-tab-content active">
               <h3 class="pdp-tab-title">Description</h3>
            ...
            <div id="tab-mfg" class="t1-tab-content">
              <h3 class="pdp-tab-title">Manufacturer Details</h3>
              <div class="t1-mfg-grid" style="display: grid; gap: 16px; margin-bottom: 24px; color: #64748b; font-size: 15px;">
                <div><strong style="color: #0f172a;">Generic Name:</strong> <br><?= $mfgGenericName !== '' ? h($mfgGenericName) : h((string)($product['name'])) ?></div>
                <div><strong style="color: #0f172a;">Country of Origin:</strong> <br><?= $mfgCountry !== '' ? h($mfgCountry) : 'India' ?></div>
                <div><strong style="color: #0f172a;">Manufacturer Name & Address:</strong> <br><?= $mfgNameAddress !== '' ? nl2br(h($mfgNameAddress)) : 'Not provided by seller.' ?></div>
              </div>
              <p style="color: #64748b; font-size: 14px; font-style: italic;">For customer support related to this product, please contact the seller via the LUXE platform or refer to the manufacturer details above.</p>
            </div>
        <div id="tab-rev" class="t1-tab-content">
          <div class="pdp-rev-layout">
            <div class="pdp-rev-summary-col">
              <div class="pdp-rev-big-score"><?= number_format($displayRating, 1) ?></div>
              <div class="pdp-rev-big-stars"><?= str_repeat('★', round($displayRating)) . str_repeat('☆', 5 - round($displayRating)) ?></div>
              <div class="pdp-rev-based-on">Based on <?= $displayReviewCount ?> reviews</div>
              <div class="pdp-rev-bars">
                <?php for($i=5; $i>=1; $i--): 
                   $pct = ($i == round($displayRating)) ? 100 : 0; 
                ?>
                <div class="pdp-rev-bar-row">
                  <span class="pdp-rev-bar-lbl"><?= $i ?>★</span>
                  ...
                  ?>
                    <div class="pdp-review-item">
                      <div class="pdp-review-head">
                        <div class="pdp-review-avatar"><?= h($rInit) ?></div>
                        <div class="pdp-review-author-info">
                          <strong><?= h($rName) ?></strong>
                          <span class="pdp-review-verified"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><polyline points="20 6 9 17 4 12"></polyline></svg>VERIFIED BUYER</span>
                        </div>
                        <div class="pdp-review-badge-wrap">
                           <div class="pdp-review-stars"><?= str_repeat('★', $rRating) . str_repeat('☆', 5 - $rRating) ?></div>
                           <span class="pdp-review-top-badge">TOP REVIEW</span>
                        </div>
                      </div>
                      ...
                      <div class="pdp-review-footer">
                        <span class="pdp-review-date"><?= date('Y-m-d H:i:s', strtotime((string)$rev['created_at'])) ?></span>
                        <button class="pdp-helpful-btn"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px;"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg> Helpful (0)</button>
                      </div>
                    </div>
                  <?php endforeach; ?>
                </div>
  ...
  <script>
    ...
    function updateStock() {
      const activeColBtn = document.querySelector('.pdp-color-swatch.active');
    ...
    function switchTab(tabId, btnElement) {
      ...
      if (btnElement) {
        ...
        const btn = Array.from(document.querySelectorAll('.t1-tab-btn')).find(b => b.getAttribute('onclick').includes(tabId));
    ...
    async function addToCartEvent() {
      ...
      try {
        ...
        if (data.ok) {
          ...
          const cartBadge = document.querySelector('a[href="cart.php"] .badge-count, a[href="cart.php"] .badge-v3');
          if (cartBadge) {
